...
public class MessageMain {
	...
	private static MessageDao medao = new MessageDao();
	...
	public static void main(String[] args) {
		...
		while (run) {
			...
			if (selectNo == 1) {
				...
				Message message_ = medao.getMessageByMid(mid);
			...
			{
				List<Message> ms = medao.getMessageList();
			...
			else if (selectNo == 3)
			{
				...
				List<Message> ms = medao.getMessageByWriter(writer);
			...
			else if (selectNo == 4)
			{
				...
				Message message_ = new Message(content, writer, null);
				
				medao.insertMessage(message_);
			}
			else if (selectNo == 5)
			{
				System.out.print("찾을 메시지 번호 입력: ");
